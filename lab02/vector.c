...
vector_t *bad_vector_new() {
  ...
  retval->data[0] = 0;
  return retval;
  // return the ptr point to stack var
}
...
vector_t also_bad_vector_new() {
  ...
  v.data[0] = 0;
  return v;
  // directrly return the copy of a stack var. works, but not good, cause it
  // would copy the whole
}
...
vector_t *vector_new() {
  ...
  if (retval->data == NULL) {
    free(retval); // Why is this line necessary? // the claimed mem wont
    allocation_failed();
  }
